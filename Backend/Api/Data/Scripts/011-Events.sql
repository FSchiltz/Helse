...
ALTER TABLE
  health.Eventtype
ADD
  COLUMN UserEditable boolean NOT NULL DEFAULT TRUE;
...
ALTER TABLE
  health.Event DROP CONSTRAINT FK_Type_TO_Event;

ALTER TABLE
  health.Event
ADD
  CONSTRAINT FK_Type_TO_Event FOREIGN KEY (Type) REFERENCES health.EventType (Id);
...
SELECT
  setval(
    pg_get_serial_sequence('health.EventType', 'id'),
    COALESCE(
      (
        SELECT
          MAX(Id) + 100
